...
public class SerializeBT {
    ...
    /*
     * 刚开始想法太死板了，只记得中序和前序两个序列才能决定一棵唯一的二叉树，
     * 于是分别进行了前序、中序遍历，中序和前序的序列用"|"分隔，之后根据分隔符分成前序和中序序列，
     * 最后采用面试题7————重建二叉树的思路进行反序列化
     * 其实遇到空指针可以也用一个字符表示，比如#，这样前序遍历序列就可以表示唯一的一棵二叉树了。
     * 而二叉树的建立，必须先要建立根结点再建立左右子树（root为空怎么调用root.left是吧），
     * 所以必须前序建立二叉树，那么序列化时也应该用前序遍历，保证了根结点在序列前面
     *
